...
class AddSalesItemTableView: UITableViewController, UITextFieldDelegate, UITextViewDelegate {
    var hud: Hud!
    ...
    override func viewWillAppear(animated: Bool) {
        NSNotificationCenter.defaultCenter().addObserver(
            self, selector: "salesItemCreated:", name: "SalesItemCreated", object: nil)
    }
    
    override func viewWillDisappear(animated: Bool) {
        NSNotificationCenter.defaultCenter().removeObserver(
            self, name: "SalesItemCreated", object: nil)
    }
    ...
    func saveNewItem() {
        if itemName.text != nil && itemQuantity.text != nil {
            ...
            DataSource.sharedTasks().createNewSalesItem(
                itemName.text,
    ...
    func salesItemCreated(notification: NSNotification) {
        hud.StopIndicator()
        hud.HudView().removeFromSuperview()
        
        delegate?.didUpdateData("add", className: "Sales")
    }
